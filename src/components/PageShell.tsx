          ...
          <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div>
              ...
              <ul className="mt-5 space-y-3">
                {productLinks.map((l) => (
                  <li key={l.href}><a href={l.href} className="text-sm text-slate-600 transition-colors hover:text-blue-600">{l.label}</a></li>
                ))}
              </ul>
            </div>
            <div>
              <h4 className="text-xs font-bold uppercase tracking-[0.2em] text-slate-400">Legal</h4>
              <ul className="mt-5 space-y-3">
                {legalLinks.map((l) => (
                  <li key={l.href}><a href={l.href} className="text-sm text-slate-600 transition-colors hover:text-blue-600">{l.label}</a></li>
                ))}
...
export function PageHeader({ eyebrow, title, lastUpdated }: { eyebrow: string; title: string; lastUpdated: string }) {
  return (
    <section className="relative overflow-hidden border-b border-slate-100 bg-gradient-to-b from-blue-50/60 via-white to-white py-16 sm:py-20">
      ...
      <div className="relative mx-auto max-w-3xl px-6 text-center">
        <span className="text-xs font-bold uppercase tracking-[0.2em] text-blue-600">{eyebrow}</span>
        <h1 className="mt-4 text-4xl font-extrabold tracking-[-0.02em] text-slate-900 sm:text-5xl">{title}</h1>
        <p className="mt-5 text-sm font-medium text-slate-500">Last updated: {lastUpdated}</p>
      </div>
    </section>
  );
}
...
export function LegalSection({ title, children }: { title: string; children: ReactNode }) {
  return (
    <section className="mt-12 first:mt-0">
      <h2 className="text-xl font-bold tracking-[-0.01em] text-slate-900 sm:text-2xl">{title}</h2>
